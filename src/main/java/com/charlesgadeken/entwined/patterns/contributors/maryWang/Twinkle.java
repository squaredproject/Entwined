    ...
    for (int i = 0; i < bright.length; i++ ) {
      if (i <= numLight) {
        if (Utilities.random(1) < 0.5f) {
          bright[i] = new SinLFO((int)Utilities.random(80, 100), 0, (int)Utilities.random(2300, 7700));
        }
        else {
          bright[i] = new SinLFO(0, (int)Utilities.random(70, 90), (int)Utilities.random(5300, 9200));
        }
      }
      else if ( i < numDarkReverse ) {
        bright[i] = new SinLFO((int)Utilities.random(50, 70), 0, (int)Utilities.random(3300, 11300));
      }
      else {
        bright[i] = new SinLFO(0, (int)Utilities.random(30, 80), (int)Utilities.random(3100, 9300));
      }
  ...
  public void run(double deltaMs) {
    ...
    for (Cube cube : model.cubes) {
      if (sparkleTimeOuts[cube.index] < Utilities.millis()) {
        ...
        sparkleTimeOuts[cube.index] = Utilities.millis() + (int)Utilities.random(11100, 23300);
      }
