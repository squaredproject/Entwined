    ...
    public void run(double deltaMS) {
        ...
        if(triggered) {
            if (!isRising) {
                for (int j = 0; j < pulleys.size(); ++j) {
                    ...
                    if (pulleys.get(j).gravity.getValuef() < 0) { //bouncebounce
                        ...
                        pulleys.get(j).gravity.setVelocity(-pulleys.get(j).gravity.getVelocityf() * Utilities.random(0.74f,0.84f));
                    }
                }
            }
            ...
            for (BaseCube cube : model.baseCubes) {
                ...
                float cHue = 0;
                for (int j = 0; j < pulleys.size(); ++j) {
                    cHue = (lx.engine.palette.getHuef() + Utilities.abs(cube.x - model.cx)*.8f + cube.transformedY*.4f + pulleys.get(j).baseHue) % 360;
                    cBrt += Utilities.max(0, pulleys.get(j).maxBrt.getValuef() * (100 - Utilities.abs(cube.transformedY/2 - 50 - pulleys.get(j).gravity.getValuef())*falloff));
                }
