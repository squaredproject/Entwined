...
public class Pond extends EntwinedTriggerablePattern {
    ...
    public void run(double deltaMs) {
        ...
        while (iter.hasNext()) {
            ...
            // Make sure the ripple's current radius plus width is inside at least one of the bounds
            // corners
            ...
            for (LXVector corner : corners) {
                if (ripple.radius - rippleWidth < ripple.origin.dist(corner)) {
                    // Rendered ripple is inside this corner of the model's bounds, so we shouldn't
                    // delete it.
                    inBounds = true;
                    break;
                }
        ...
        // If we aren't at maxRipples, create a new ripple if a random number is inside bounds
        // defined by
