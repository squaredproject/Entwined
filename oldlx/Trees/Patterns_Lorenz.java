...
class Fountain extends TSPattern {
  enum State {
  ...
  // Randomly pickked index of the next tree used to update the color.
  private int treeIndexNewColor = 0;

  // Radial distance from tree with new color used to update colors of shrubs. This
  // distance increases lineraily during the UPDATE_SHRUBS_HUE state.
  ...
  // x/z global coorindates of the tree that is changing colors. This is the position
  // from which to spread the new color to all shrubs.
  private float newColorTreeX = 0;
  private float newColorTreeZ = 0;

  // x/z global ccordinates of the three trees.
  ...
  final BasicParameter waveParam = new BasicParameter("Wave", 4, 1, 10);

  // Height of the smaller ripple wave running over the shrubs in z-direction.
  final BasicParameter rippleParam = new BasicParameter("Ripple", 0.8, 0, 5);
  ...
  Fountain(LX lx) {
    ...
    for (BaseCube cube : model.baseCubes) {
      if (cube.treeOrShrub == TreeOrShrub.SHRUB) {
        ...
        // Find the world position of each tree. (Is there a way to get this info
        // in an easier way? Maybe from a config file?)
        // Only consider cubes with non-zero local position.
        if (cube.sculptureIndex < 3 && cube.sx > 1) {
          treesX[cube.sculptureIndex] = cube.x - cube.sx;
          treesZ[cube.sculptureIndex] = cube.z - cube.sz;
        }
      }
    }
    ...
    for (BaseCube cube : model.baseCubes) {
      if (cube.treeOrShrub == TreeOrShrub.TREE) {
        if (cube.sculptureIndex == treeIndexNewColor) {
          ...
          float h = shrubHue;
          if (rUpdateShrubColor > cube.gr) {
            float fade = Math.min(2000, rUpdateShrubColor - cube.gr) / 2000;
