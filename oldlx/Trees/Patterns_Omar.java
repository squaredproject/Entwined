...
class FairySnakes extends TSPattern {
 ...
 */
  HashMap<String, List<BaseCube>> snakePaths;
  HashMap<String, List<BaseCube>> snakePathsAlternate;
  int snakeLength = 10;
  double counter = 0;
  List<SnakeConfig> snakeConfigs;
  HashMap<String, Integer> numSnakesPerCircle;

  final BasicParameter speedParam = new BasicParameter("Speed", 3, 0.1, 20);
  ...
  FairySnakes(LX lx) {
    ...
    snakePaths = new HashMap<String, List<BaseCube>>();
    snakePathsAlternate = new HashMap<String, List<BaseCube>>();
    snakeConfigs = new ArrayList<SnakeConfig>();
    numSnakesPerCircle = new HashMap<String, Integer>();

    HashMap<String, List<BaseCube>> outerPaths = new HashMap<String, List<BaseCube>>();
    HashMap<String, List<BaseCube>> innerPaths = new HashMap<String, List<BaseCube>>();
    ...
    for (FairyCircle fc : model.fairyCircles) {
      List<BaseCube> newInnerPath =  new ArrayList<BaseCube>();
      List<BaseCube> newOuterPath =  new ArrayList<BaseCube>();
    ...
    for (Map.Entry<String, List<BaseCube>> entry : outerPaths.entrySet()) {
      List<BaseCube> finalPath = new ArrayList<BaseCube>();
      List<BaseCube> finalPathAlt = new ArrayList<BaseCube>();

      String pieceId = entry.getKey();

      List<BaseCube> outerPath = entry.getValue();
      List<BaseCube> innerPath = innerPaths.get(pieceId);
  ...
  void makeSnake(String pieceId, float hue, int offset, float fadeFactor, 
    ...
    List<BaseCube> path = snakePaths.get(pieceId);
    if (alternate == true) {
  ...
  public void run(double deltaMs) {
    ...
    for (Map.Entry<String, Integer> entry : numSnakesPerCircle.entrySet()) {
      ...
      String pieceId = entry.getKey();
      float fadeFactor = snakeFadeInSin.getValuef() * (float)num;
