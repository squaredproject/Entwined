...
// The canopy server is delegated with security ( like checking location )
// Since Socket.io is real-time (we'll see!), and bidirectional, the Canopy
...
// or you receive events with socket.on("eventType", new Emitter.Listener() ...
// that's it. According to the "contract", if an event is emitted while disconnected,
// they will be buffered. Probably, from Canopy to Lx, we should probably have a timestamp
...
class CanopyController {
    ...
    CanopyController(Entwined engine) {
        ...
        if ((Config.canopyServer == null) || (Config.canopyServer.length() == 0)) {
            System.out.println("Canopy server disabled");
            enabled = false;
            return;
        }
        System.out.println("Canopy server enabled: canopy URL "+Config.canopyServer);
        enabled = true;
        ...
        canopyRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (!enabled) {
                    System.out.println("Canopy server not enabled, return");
                    return;
                }
