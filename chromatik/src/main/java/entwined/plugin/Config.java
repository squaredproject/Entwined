...
public final class Config {
    ...
    // The iPad system, which allows control by an outside server
    // Now, more cleanly done natively
    public static Boolean IpadServerEnable = true;

    // the NFC interaction server
    public static Boolean NFCServerEnable = false;
    public static int NFCNumActivities = -1;
    public static short NFCPort = -1;
    // Attract Mode is a feature of NFC server, runs if no one is interacting
    ...
    static void Init(String configFilename) {
        ...
        try (FileReader fr = new FileReader(configFile)) {
            ...
            if (obj.has("IpadServerEnable")) {
                IpadServerEnable = obj.get("IpadServerEnable").getAsBoolean();
                System.out.println("Config: Ipad server set to " + IpadServerEnable);
            }
