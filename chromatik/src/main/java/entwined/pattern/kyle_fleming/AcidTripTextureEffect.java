  ...
  public void run(double deltaMs, double strength) {
    if (amount.getValue() > 0) {
      for (int i = 0; i < colors.length; i++) {
        ...
        float newHue = EntwinedUtils.abs(model.cy - CubeManager.getCube(cube.index).localY) + EntwinedUtils.abs(model.cy - CubeManager.getCube(cube.index).localTheta) + trails.getValuef() % 360;
