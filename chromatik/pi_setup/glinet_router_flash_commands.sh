...
sed -i "s/ifname 'eth0.1'/ifname 'eth0.1 eth0.2'/g" /etc/config/network
#only replace the first occurence of 192.168.*.* in this file
grep 10.0.1.1 /etc/config/network > /dev/null; if [ $? -eq 1 ]; then sed -i "s/`grep 192.168 /etc/config/network  | head -n1`/\toption ipaddr '10.0.1.1'/g" /etc/config/network ; fi
